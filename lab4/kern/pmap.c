...
//----------------------------------------  Lab3  ------------------------------------------------------------
	...
	//cprintf("check_page_free_list finished\n");
	check_page_alloc();
	//cprintf("check_page_alloc finished\n");
	check_page();
...
//----------------------------------------My Code-------------------------------------------------------------
	...
	size_t mmiopages = MPENTRY_PADDR / PGSIZE;
	//cprintf("MPENTRY_PADDR: %u --- %u --- %u\n", usedpages, mmiopages, npages_basemem);
	//It shows mmiopages < npages_basemem < usedpages
	page_free_list = NULL;
	pages[0].pp_ref = 1;
...
//------------  Lab4  ----------------------------------------------------------------------------------------		
	uintptr_t res;
	res = ROUNDUP(base + size, PGSIZE);
	if (res > MMIOLIM)
		panic("mmio_map_region failed!\n");
	res = base;
	base += ROUNDUP(size, PGSIZE);
	pa = ROUNDDOWN(pa, PGSIZE);
	boot_map_region(kern_pgdir, res, ROUNDUP(size, PGSIZE), pa, PTE_W | PTE_PCD | PTE_PWT);
	return (void *)res;
//------------  Lab4  ----------------------------------------------------------------------------------------		
